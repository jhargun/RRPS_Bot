'''
This file trains the model used by LstmAgent. Note that unlike other files, it requires a GPU to run
(training will take far too long on a CPU). I recommend against running this code (even on a GPU, it
takes hours to train, and I have not provided the data used which takes an additional few hours to generate).
I have just provided this file for reference. Please let me know if you want the data used to run this
code yourself.
...
# For window size 200 with no multiprocessing
WINDOW_SIZE = 200   # Size of the window to use for the LSTM
NUM_WINDOWS_PER_SERIES = 1000 - WINDOW_SIZE  # Number of windows per time series
